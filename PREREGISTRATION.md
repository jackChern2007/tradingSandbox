...
| 2026-09-06 | *(after tag `prereg-v1`)* Tokenizer/decoder check **run once** and its record table in `elicitation.md` filled in; result files committed. First attempt against Ollama's internal worker failed (forced ChatML template, thinking on) and is kept as evidence; the run of record is against a frozen standalone `llama-server` (build `b1-b4d6c7d8f`, `--jinja`) on the same weights. `tokencheck.py` exited 1 solely because `<think>` appears in the top-20 at p = 4e-05; Jack accepted it as the pass on the `elicitation.md` criterion ("dominated by Yes/No", mass median 0.9974). No policy, prompt, accepted-token set, or floor changed. | Yes |
